...
public class Implication {
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
		...
		Job job=Job.getInstance(conf,"Implication"); 
		job.setJarByClass(Implication.class);
		...
		Path outputPath = new Path("/StackOverflow/result/Implication");
	...
	public static class MyReducer extends Reducer<Text,IntWritable, Text,NullWritable>{
		...
		protected void cleanup(Reducer<Text, IntWritable, Text, NullWritable>.Context context)
				throws IOException, InterruptedException {
			Text text=new Text();
			System.err.println(sumy+"   "+sumn);
			text.set(String.format("Yes: %.2f%%\nNo: %.2f%%", sumy/sum*100,sumn/sum*100));
